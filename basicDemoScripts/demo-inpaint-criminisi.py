...
raw_ebsd = plt.imread('./jupiter.jpg')/255.
plt.imshow(raw_ebsd); plt.title('clean'); plt.show()

damage = plt.imread('./mask2.png').astype('bool'); plt.imshow(damage/1); plt.show()

dmg_ebsd = np.copy(raw_ebsd)
dmg_ebsd[~damage] = 1
plt.imshow(dmg_ebsd); plt.title('damaged'); plt.imsave('./damaged_mask2.png', dmg_ebsd); plt.show()
...
# plt.imshow(our_result); plt.title('our result'); plt.imsave('./crimininsi_modified_mask2.png', our_result); plt.show()


# crim_result = ebsd.criminisi.inpaint(dmg_ebsd, mask, psz, restrict_search = False)
# plt.imshow(crim_result); plt.title('crim result'); plt.imsave('./crimininsi_mask2.png', crim_result); plt.show()

pconv = plt.imread('./pconv_mask2.jpg')/255.
dmg_ebsd[~damage] = pconv[~damage]
hybrid_result = ebsd.criminisi.inpaint(dmg_ebsd, mask, psz, restrict_search = True)
plt.imshow(hybrid_result); plt.title('hybrid result'); plt.imsave('./hybrid_mask2.png', hybrid_result); plt.show()
